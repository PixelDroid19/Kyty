...
static void z_check(const HW::DepthRenderTarget& z)
{
	...
	if (z.z_info.format != 0 || z.stencil_info.format != 0)
	{
		...
		// TILE_STENCIL_DISABLE + null plane ⇒ depth-only (ResolveEffectiveStencilFormat).
		const uint32_t effective_stencil = State::ResolveEffectiveStencilFormat(z);
		EXIT_NOT_IMPLEMENTED(effective_stencil != 0 && z.stencil_write_base_addr == 0);
                                 ...
                                 uint32_t index_count, uint32_t flags)
{
	...
	{
		case 4: topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST; break;   // kPrimitiveTypeTriList
		case 5: topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_FAN; break;    // kPrimitiveTypeTriFan (same as Gen4 DrawIndex)
		case 6: topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP; break;  // kPrimitiveTypeTriStrip
